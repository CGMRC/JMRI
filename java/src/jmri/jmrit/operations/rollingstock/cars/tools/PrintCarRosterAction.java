...
public class PrintCarRosterAction extends AbstractAction {
    ...
    private void printCars() {
        ...
        try {
            ...
            for (Car car : cars) {
                ...
                // car number
                number = padAttribute(car.getNumber().trim(), 8);
                // car road
                ...
                if (printCarOwner.isSelected()) {
                    owner = padAttribute(car.getOwnerName().trim(), 6);
                ...
                if (printCarBuilt.isSelected()) {
                    ...
                    built = padAttribute(car.getBuilt().trim(), 7);
                }
                ...
                String s = number +
                        ...
                        comment;
                        */
                String s = owner + road + number + built + type + color + length + weight + comment;

                if (s.length() > numberCharPerLine) {
                    s = s.substring(0, numberCharPerLine);
                }
                //log.info("writing line: " + s);
                writer.write(s + NEW_LINE);
            }

            log.info("Forcing print...");
    ...
    private void printTitleLine(HardcopyWriter writer) throws IOException {
        String s = padAttribute("Owner", 6) +
            padAttribute("Road", InstanceManager.getDefault(CarRoads.class).getMaxNameLength()) +
            padAttribute("Number", 8) +
            padAttribute("Insp", 7) +
            padAttribute("Type", InstanceManager.getDefault(CarTypes.class).getMaxFullNameLength()) +
            padAttribute("Color", InstanceManager.getDefault(CarColors.class).getMaxNameLength()) +
            padAttribute("Len", Control.max_len_string_length_name) +
            padAttribute("Wgt", Control.max_len_string_weight_name) +
            padAttribute("Comment", 30);
        /*
    ...
    public class CarPrintOptionFrame extends OperationsFrame {
        ...
        public CarPrintOptionFrame(PrintCarRosterAction pcr) {
            ...
            fontSizeComboBox.setSelectedItem(9);
            ...
            printCarComment.setSelected(true);
        ...
        private void loadSortByComboBox(JComboBox<String> box) {
            ...
            box.setSelectedItem(panel.carsTableModel.getSortByName(panel.carsTableModel.SORTBY_OWNER));
        }
